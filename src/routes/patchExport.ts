 ...
 *   1. Instrument roles (乐器-*) are always exported.
 ...
 */
import { type Release, pagenoJoin, multiDiscPageNoJoin } from "./postprocess";
...
function isInstrumentRole(roleID: string): boolean {
    return roleID === "乐器" || roleID.startsWith("乐器-");
}
...
function shouldExport(
    ...
    // 1. Instrument roles are always exported.
    if (isInstrumentRole(roleID)) return true;
